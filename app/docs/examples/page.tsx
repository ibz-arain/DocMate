...
export default function ExampleInvoicePage() {
  return (
    <div className="p-6 prose prose-gray dark:prose-invert max-w-none">
      ...
      <p className="text-lg text-muted-foreground mb-8">
        Let's walk through a common scenario: extracting key information from an invoice using DociMate. This example shows how you can target specific data points and view them in different formats, all handled automatically.
      </p>
      ...
      <h2 className="text-2xl font-semibold mt-10 mb-4">Using DociMate to Extract Specific Data</h2>
      <p className="text-muted-foreground mb-6">
        Instead of reading the whole invoice, we can tell DociMate exactly what we need using a template in the <Link href="/playground" className="text-primary hover:underline">Playground</Link>.
      </p>
      ...
      />
      <p className="text-muted-foreground mb-4">
        By defining only these fields, we instruct DociMate to focus its extraction efforts solely on finding this specific information, ignoring the rest.
      </p>

      <h3 className="text-xl font-semibold mt-6 mb-3">Step 3: Process and View Results</h3>
      <p className="text-muted-foreground mb-4">
        After hitting 'Process', DociMate uses the template to analyze the invoice. Within moments, it presents *only* the data we asked for in various formats.
      </p>
  ...
  "DueDate": "2023-12-31"
      ...
      <p className="text-muted-foreground mb-6">
        This example highlights several key benefits of DociMate:
      </p>
      ...
      <p className="text-lg text-muted-foreground mt-10">
        This is just one example. You can apply the same principles – defining targeted templates – to extract specific information from contracts, reports, forms, and many other document types using DociMate.
      </p>
    </div>
  );
} 